...
#
# time = np.linspace(1, 96, 96)
# x_weekday_total = X_input[0,0:5,0:96]
# x_weekend_total = X_input[6,0:5,0:96]
#
# for i in range (5):
#     plt.plot(time,x_weekday_total[i])
# plt.xlabel('15-min interval')
# plt.ylabel('User Energy Consumption (KW)')
# plt.savefig('real_usage_weekday')
# plt.show()
#
# for j in range(5):
#     plt.plot(time,x_weekend_total[j])
# plt.xlabel('15-min interval')
# plt.ylabel('User Energy Consumption (KW)')
# plt.show()
# plt.savefig('real_usage_weekend')



# time = np.linspace(1, 96, 96)
# synthetic_filename_week = 'filter_free_weekday_5000_s1.csv'
# df_synthetic_week = pd.read_csv(synthetic_filename_week).to_numpy()
#
#
# indices_to_plot = [1,5,9,10,11]
# for i in indices_to_plot:
#     current_week_index = i*96-1
#     plt.plot(time,df_synthetic_week[current_week_index:current_week_index+96], label = i)
# plt.xlabel('15-min interval')
# plt.ylabel('User Energy Consumption (KW)')
# # plt.legend()
# plt.savefig('synthetic_usage_weekday')
# plt.show()
#
#
#
...
#
#
# synthetic_filename_weekend = 'filter_free_weekend_5000_s1.csv'
# df_synthetic_weekend = pd.read_csv(synthetic_filename_weekend).to_numpy()
# indices_to_plot = [1,2,4,5,9]
# for j in indices_to_plot:
#     current_weekend_index = j*96-1
#     plt.plot(time,df_synthetic_weekend[current_weekend_index:current_weekend_index+96], label = j)
# plt.xlabel('15-min interval')
# plt.ylabel('User Energy Consumption (KW)')
# plt.legend()
# plt.savefig('synthetic_usage_weekend')
#
# plt.show()

time = np.linspace(1, 96, 96)
synthetic_filename_week = 'filter_free_weekday_5000_s1.csv'
df_synthetic_week = pd.read_csv(synthetic_filename_week,header=None).to_numpy()

weekday_n = 30
interval_day = 96
X_input_single_day = np.zeros((weekday_n, interval_day))
for i in range(weekday_n):
    for j in range(interval_day):
        X_input_single_day[i, j] = df_synthetic_week[j + i * interval_day]


synthetic_filename_weekend = 'filter_free_weekend_5000_s1.csv'
df_synthetic_weekend = pd.read_csv(synthetic_filename_weekend,header=None).to_numpy()

weekend_n = 70
interval_day = 96
X_input_single_weekend = np.zeros((weekend_n, interval_day))
for i in range(weekend_n):
    for j in range(interval_day):
        X_input_single_weekend[i, j] = df_synthetic_weekend[j + i * interval_day]

concatenated_array = np.concatenate((X_input_single_day, X_input_single_weekend), axis=0)
# Sum along the first axis (rows) to get a (96,) array
summed_array = np.sum(concatenated_array, axis=0)

# Reshape the resulting array to (1, 96)
# summed_array = summed_array.reshape(1, 96)
#     X_input[image_index, :, 0:96] = X_input_single_day
# X_input = X_input.astype(np.float32)

plt.plot(time,summed_array)
plt.xlabel('15-min interval')
plt.ylabel('Total User Energy Consumption (KW)')
# plt.legend()
plt.savefig('total_usage_weekday (30-70)')
plt.show()
# for i in 30:
#     for j in 96:
#         result[j] = df_synthetic_week[]
#     current_week_index = i*96-1
#     plt.plot(time,df_synthetic_week[current_week_index:current_week_index+96], label = i)
# plt.xlabel('15-min interval')
# plt.ylabel('User Energy Consumption (KW)')
