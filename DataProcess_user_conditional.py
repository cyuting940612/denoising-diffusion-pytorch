...
def data_process():
    ...
    # climate_data_week_repeated = np.tile(temp_re, (1, user_num, 1))
    #
    # # Reshape both user_data and climate_data_repeated
    # user_data_week_reshaped = user_re.reshape(-1, 96)
    # climate_data_week_reshaped = climate_data_week_repeated.reshape(-1, 96)
    #
    # final_output = np.zeros((user_data_week_reshaped.shape[0],1,interval_day))

    # final_output[:,0,:],min1,max1 = minmax_scaling_per_feature(climate_data_week_reshaped)
    final_output,min2,max2 = minmax_scaling_per_feature(user_re)
